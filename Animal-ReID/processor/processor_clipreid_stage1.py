...
def do_train_stage1(cfg,
    ...
    """
    # id별로 feature 계산하기
    id_to_features = collections.defaultdict(list)
    logger.info("id별로 feature 계산")
    with torch.no_grad():
        for n_iter, (img, vid, target_cam, target_view) in enumerate(train_loader_stage1):
            img = img.to(device)
            target = vid.to(device)
            with amp.autocast(enabled=True):
                ...
                for i, img_feat in zip(target, image_features):
                    id_to_features[i.item()].append(img_feat.cpu())
    
    # Compute average text features for each id
    aggregated_image_features = {}
    for img_id, features in id_to_features.items():
        aggregated_image_features[img_id] = torch.mean(torch.stack(features), dim=0)
    
    labels_list = torch.tensor(list(aggregated_image_features.keys()), dtype=torch.int64).cuda()
    image_features_list = torch.stack(list(aggregated_image_features.values()), dim=0).cuda()

    batch = cfg.SOLVER.STAGE1.IMS_PER_BATCH
    num_image = labels_list.shape[0]
    i_ter = num_image // batch
    del id_to_features, aggregated_image_features
